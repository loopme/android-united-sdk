...
public class ResponseParser {
    ...
    public static GetResponse<GdprResponse> parseGdprResponse(HttpRawResponse rawResponse) {
        ...
        if (rawResponse.getCode() == HttpURLConnection.HTTP_OK) {
            if (isGdprBodyParcelable(rawResponse.getBody())) {
                GdprResponse gdprResponse = convertToGdprResponse(rawResponse.getBody());
                response.setCode(rawResponse.getCode());
                response.setBody(gdprResponse);
            } else {
                response.setMessage(Errors.ERROR_MESSAGE_BROKEN_SERVERS_RESPONSE);
            }
    ...
    private static boolean isGdprBodyParcelable(byte[] body) {
        return body != null && isParcelableAsJson(body) && isParcelableAsGdprResponse(body);
    }

    private static boolean isParcelableAsGdprResponse(byte[] body) {
        return convertToGdprResponse(body) != null;
    }

    private static GdprResponse convertToGdprResponse(byte[] body) {
        try {
            return new GdprResponseParser().parse(body);
        } catch (JSONException e) {
            Logging.out(ResponseParser.class.getSimpleName(), e.getMessage());
        }
        return null;
