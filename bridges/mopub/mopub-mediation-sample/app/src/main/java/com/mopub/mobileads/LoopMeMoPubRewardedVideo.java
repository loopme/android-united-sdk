...
public class LoopMeMoPubRewardedVideo extends CustomEventRewardedVideo implements LoopMeInterstitial.Listener {
    ...
    protected boolean checkAndInitializeSdk(@NonNull Activity launcherActivity,
        ...
        MoPubLog.log(MoPubLog.AdapterLogEvent.CUSTOM, LOG_TAG, "Bridge checkAndInitializeSdk");

        mLoopMeAppId = serverExtras.get(KEY_EXTRA_LOOPME_APPKEY);

        if (!isAppKeyValid()) {
            MoPubLog.log(MoPubLog.AdapterLogEvent.CUSTOM, LOG_TAG, "App key should not be null");
            return false;
        }
    ...
    protected void loadWithSdkInitialized(@NonNull Activity activity,
                                          @NonNull Map<String, Object> localExtras,
                                          @NonNull Map<String, String> serverExtras) {
        MoPubLog.log(MoPubLog.AdapterLogEvent.CUSTOM, LOG_TAG, "Bridge loadWithSdkInitialized");
    ...
    protected void showVideo() {
        if (hasVideoAvailable()) {
            ...
            MoPubLog.log(MoPubLog.AdapterLogEvent.CUSTOM, LOG_TAG, "rewarded video is not ready");
        }
    }
    ...
    private int getInt(String amount) {
        try {
            ...
            e.printStackTrace();
            MoPubLog.log(MoPubLog.AdapterLogEvent.CUSTOM, LOG_TAG, "Rewarded amount is not of type int");
        }
        return NO_AMOUNT;
    }
